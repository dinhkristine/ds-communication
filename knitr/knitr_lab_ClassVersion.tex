...
\hlstd{fm.table} \hlkwb{<-} \hlkwd{xtable}\hlstd{(fm1,} \hlkwc{digits}\hlstd{=}\hlnum{2}\hlstd{,}
                   \hlkwc{caption} \hlstd{=} \hlstr{"Summary of Regression"}\hlstd{,}
...
% Date and time: Thu, Sep 03, 2020 - 3:41:24 PM
...
\caption{Plot of Regression Diagnostics}
\label{reg_diag}
\end{center}
\end{figure}

% Write a short blurb citing the figure and stating what it is.
As seen in Figure~\ref{reg_diag}, residuals vs fitted values have a pattern similar to a fan.
...
\lstinputlisting[language=R, caption = List of codes]{knitr-lab-codes.R}
