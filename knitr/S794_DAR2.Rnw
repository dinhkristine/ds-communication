...
tidy %<>% 
  xtable(caption = "Summary regression of final model with three variables: results of digial rectal exam ('dpros'), PSA scores ('psa'), and total Gleason scores ('gleason'). The columns represent the features names, coefficient estimated values, standard errors, test statistics, significant p-value, odd ratios, 95\% lower confident interval of odd ratios, and 95\% upper confident interval of odd ratios, respectively from left to right.", 
...
stargazer(prostate %>% dplyr::select(-id, -psa_bins, -capsule), 
          title = "Summary Statistics for all independent features", 
          label="descrips",
          table.placement = "H")
@
\end{center} 
