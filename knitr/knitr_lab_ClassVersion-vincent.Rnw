...
By using {\tt Sexpr} function, we can show there are \Sexpr{numvars} variables in the data set. We can present the predicted values for new schools = \Sexpr{newpred1} and \Sexpr{newpred2}.
...
x <- data.frame(univ
                ...
                , lwr
                , upr
                , bind = rbind(new1[,"Outstate"], new2[,"Outstate"]))
...
% Recall that this is straight LaTeX, no knitr code chunk needed!
  ...
  \lstinputlisting[language=R, caption = Code used for this lab exercise]{knitr_lab_ClassVersion.Rnw}
