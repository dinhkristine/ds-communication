...
#### Functions ####
ScatterPlotFunction <- function(df, xvar, xlab, smooth_method = "loess", number_plot){
...
p2 <- ScatterPlotFunction(ny2, "lpop", "Log-Population", number_plot = "(2)")

p3 <- ScatterPlotFunction(ny2, "ldens", "Log-Density", number_plot = "(3)")
...
p1 <- ScatterPlotFunction(set2, "lwealth", "Log-Wealth", "lm", number_plot = "(1)")
p2 <- ScatterPlotFunction(set2, "lincome", "Log-Income", "lm", number_plot = "(2)")
...
fit5 <- lm(lexpen ~ lwealth + lpop + lpint + lincome, data = set2)
...
colnames(fit_final_table) <- c("Term", "Coef", "SdError", "F-Stat", 
                               "pValue", "10% Coef", "2.5% CI", "97.5% CI", "VIF")
...
wm_tab <- wm %>%
  filter(year != 1992) %>% 
  dplyr::select("Town" = town, "Year" = year, 
                "Predicted Expen" = fit, 
                "Lower PI" = lwr, "Upper PI" = upr) %>% 
  xtable(caption = "Prediction table of two municipalities in New York (Warwick and Monroe)", 
         label = "pred_tab",
         table.placement="H", 
         digits = 1)
...
stargazer(ny2 %>% dplyr::select(expen, wealth, pop, pint, dens, income, growr), 
...
reg_table <- bind_rows(fit1_summary, fit2_summary, fit3_summary, fit4_summary, fit5_summary) %>% 
  as.data.frame() %>% 
  xtable(digits = 3,
...
set2 %>% 
  ...
  xtable(caption = "Outlier Obsevations", 
       label = "outlier_obs",
       table.placement="H")
...
p1 <- ScatterPlotFunction(set2, "lpop", "Log-Population", "lm", number_plot = "(1)")
p2 <- ScatterPlotFunction(set2, "lpint", "% Log Intergovernmental Funds", "lm", number_plot = "(2)")
p3 <- ScatterPlotFunction(set2, "ldens", "Log-Density", "lm", number_plot = "(3)")
p4 <- ScatterPlotFunction(set2, "lgrowr", "Log-Growth Rate", "lm", number_plot = "(4)")
